...
export async function POST(request) {
  const transaction = startSentryTransaction("Send Paid Fax", "sendFax");
  ...
  try {
    ...
    if (!response.ok) {
      ...
      captureApiError(error, "sendFaxPaid", { status: response.status });
      throw error;
    }
    ...
    captureApiError(error, "sendFaxPaid");
